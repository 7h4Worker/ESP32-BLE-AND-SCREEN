...
//******************************************************************************** */
// 未连接界面*************************************************************************/
static lv_obj_t* create_ui_disconnected(void) {
...
// 连接中界面*************************************************************************/
static lv_obj_t* create_ui_connecting(void) {
    lv_obj_t *scr = lv_obj_create(lv_scr_act());
    lv_obj_center(scr);
    lv_obj_clear_flag(scr, LV_OBJ_FLAG_SCROLLABLE);
...
//已连接界面*************************************************************************/
static lv_obj_t* create_ui_connected(void)
{
...
LV_FONT_DECLARE(GenJyuuGothic_32);//字体
static lv_obj_t *ui_disconnected = NULL; // 未连接界面
static lv_obj_t *ui_connecting = NULL; // 连接中界面
static lv_obj_t *ui_connected = NULL; // 已连接界面
...
static void update_ui(void)
{
    ...
    // 先全部隐藏
    lv_obj_add_flag(ui_disconnected, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(ui_connecting, LV_OBJ_FLAG_HIDDEN);
    lv_obj_add_flag(ui_connected, LV_OBJ_FLAG_HIDDEN);

    // 根据状态显示对应界面
    switch (conn_status) {
        case CONN_STATUS_DISCONNECTED:
            lv_obj_clear_flag(ui_disconnected, LV_OBJ_FLAG_HIDDEN);
            break;
        case CONN_STATUS_CONNECTING:
            lv_obj_clear_flag(ui_connecting, LV_OBJ_FLAG_HIDDEN);
            break;
...
//任务1+2：更新UI和脑电数据
static void ui_and_data_update_task(void *arg)
{
    while (1) {
        // 更新UI
        update_ui();
        update_prama();
        vTaskDelay(pdMS_TO_TICKS(1000)); // 每100ms跑一次
    }
}
...
void app_main(void)
{
    ...
    ui_disconnected = create_ui_disconnected(); // 创建未连接界面
    ui_connecting = create_ui_connecting();     // 创建连接中界面
    ui_connected = create_ui_connected();       // 创建已连接界面
    ...
    // 已连接时才启动刷新任务
    xTaskCreate(ui_and_data_update_task, "ui_and_data_update_task", 4096, NULL, 5, NULL);
